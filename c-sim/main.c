...
void dynamicModelTask(void *pvParameters) {
    ...
    while(1) {
        ...
        // local_acc[0] = 0.0 - sin(x1)*sin(x3) - cos(x1)*cos(x3)*sin(x2) + noise_x;
        // local_acc[1] = 0.0 + cos(x3)*sin(x1) - cos(x1)*sin(x2)*sin(x3) + noise_y;
        // local_acc[2] = t*(motors[0] + motors[1] + motors[2] + motors[3])/0.06/9.81 + cos(x1)*cos(x2)
        //     + noise_z;
...
int main(int argc, char **argv) {
    ...
    xTaskCreate(referenceGenTask, "Reference Generator Task",
                configMINIMAL_STACK_SIZE, (void*)&r_params, 2, NULL);
    ...
    xTaskCreate(filterTask, "Filter Task",
                configMINIMAL_STACK_SIZE, (void*)&f_params, 6, NULL);
    ...
    xTaskCreate(controlSystemTask, "Control System Task",
                configMINIMAL_STACK_SIZE, (void*)&c_params, 4, NULL);
    // xTaskCreate(controlInputTask, "Control System Reading Task",
    //         configMINIMAL_STACK_SIZE, (void*)&c_params, 5, NULL);
    // xTaskCreate(controlCalTask, "Control System Computing Task",
    //         configMINIMAL_STACK_SIZE, (void*)&c_params, 4, NULL);
    // xTaskCreate(controlOutputTask, "Control System Writing Task",
    //         configMINIMAL_STACK_SIZE, (void*)&c_params, 3, NULL);
